...
// A list URL is acceptable if it's http(s), embeds no credentials, and resolves
// to a public address. Mirrors the checks on the single-add schema below.
function isAcceptableRemoteUrl(value: string): boolean {
  try {
    const parsed = new URL(value);
    if (!/^https?:$/i.test(parsed.protocol)) return false;
    if (parsed.username || parsed.password) return false;
    return !isUnsafeRemoteUrl(value);
  } catch {
    return false;
  }
}

// Give each bulk-added source a readable, distinct name. Most community lists
// live on GitHub where every raw URL shares one host, so lead with the owner (or
// owner/repo) from the path and fall back to the host.
function deriveListName(value: string): string {
  try {
    const { host, pathname } = new URL(value);
    const segments = pathname.split('/').filter(Boolean);
    if (/github/i.test(host) && segments.length > 0) {
...
// Subscribe to many lists at once. `input` is free text: one URL per line
// (blank lines and `#` comments ignored). A single URL that turns out to be a
// "list of lists" (a plain-text manifest of list URLs, like nzbdavex's
// warden_list) is expanded into one source per child URL. Every resulting source
// gets the same trust and refresh interval.
const BulkAddSchema = z.object({
  input: z.string().min(1).max(512 * 1024),
  trust: Trust.optional(),
  refreshSeconds: z.number().int().min(1).max(2592000).optional(),
  // Set false to add a single manifest-looking URL verbatim instead of expanding.
...
router.post(
  ...
  async (req, res, next) => {
    ...
    if (!parsed.success) return next(badRequest('Provide one or more list URLs.'));
    const { input, trust, refreshSeconds, expand } = parsed.data;
    try {
      const lines = input
        .split(/\r?\n/)
        .map((l) => l.trim())
        .filter((l) => l !== '' && !l.startsWith('#'));
      const invalid = lines.filter((l) => !isAcceptableRemoteUrl(l)).length;
      let urls = [...new Set(lines.filter(isAcceptableRemoteUrl))];
      if (urls.length === 0) {
        return next(badRequest('No valid http(s) list URLs found.'));
      }

      // A single URL might be a manifest of list URLs — expand it into its
      // children. Multiple pasted URLs are each treated as their own list.
      let expandedFrom: string | undefined;
      let truncated = false;
      if (urls.length === 1 && expand !== false) {
        const children = await ScreenerRemoteSourceService.expandManifest(urls[0]);
        if (children && children.length > 0) {
          expandedFrom = urls[0];
          const valid = [...new Set(children.filter(isAcceptableRemoteUrl))];
          truncated = valid.length < children.length;
          urls = valid;
        }
      }

      const existing = new Set(
        (await ScreenerStore.getSources())
          .map((s) => s.url)
      ...
      // Fetch the new sources in the background so a large batch doesn't hold the
      // response open; the dashboard reflects their status as it polls.
      if (added.length > 0) {
        void ScreenerRemoteSourceService.refreshByIds(added).catch((err) =>
          logger.debug(`bulk refresh failed: ${err}`)
        );
      }
