...
function decodeAndAuthorizeRequest(
  ...
  } else if (parts.length === 3 && (parts[0] as string) === 'e') {
    encodeMode = 'e';
    [, encodedAuth, encodedData] = parts;
  } else if (parts.length === 4 && (parts[0] as string) === 'u') {
    encodeMode = 'u';
    [, encodedAuth, encodedData, signature] = parts;
  } else {
    throw new APIError(
  ...
  if (encodeMode === 'e') {
    ...
    // 'u' mode skips encryption but must still prove it was generated by
    // this server, not hand-crafted by a caller who only holds a
    // low-privilege 'proxy' credential — otherwise data.url below would be
    // fully attacker-controlled, an SSRF into any address this server can
    // reach.
    if (
      !signature ||
      !verifyUnencryptedProxyToken(encodedAuth, encodedData, signature)
    ) {
      logger.warn(`[${requestId}] Invalid or missing 'u' mode signature`);
      throw new APIError(
