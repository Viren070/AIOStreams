...
export async function serveUsenetStreamToken(
  ...
  opts: {
    ...
    /**
     * Send a failed stream to a static clip instead of a JSON body, so a player
     * shows something explanatory rather than raw text. The HTTP stream route
     * sets it; WebDAV does not, because a client that asked for a byte range
     * has no use for a 302 to an mp4.
     */
  ...
  try {
    ...
    if (err instanceof DebridError) {
      ...
      if (opts.redirectOnError && req.query.download === undefined) {
