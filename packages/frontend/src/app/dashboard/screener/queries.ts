import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import { api } from '@/lib/api';

export type Trust = 'full' | 'corroborate' | 'observe';
export type SourceKind = 'local' | 'remote' | 'imported';

export interface ScreenerSource {
  id: string;
  kind: SourceKind;
  name: string;
  url: string | null;
  enabled: boolean;
  trust: Trust;
  refreshSeconds: number;
  lastChecked: number;
  lastUpdated: number;
  status: string | null;
  count: number;
}

export interface ScreenerSettings {
  backboneScope: boolean;
  trustedBackbones: string[];
}

export interface ScreenerSnapshot {
  counts: { total: number; local: number };
  sources: ScreenerSource[];
  settings: ScreenerSettings;
}

const ROOT = ['screener'] as const;

export function useScreener() {
  return useQuery({
    queryKey: ROOT,
    queryFn: () => api<ScreenerSnapshot>('/screener'),
    staleTime: 10_000,
  });
}

function useScreenerMutation<TArgs>(
  fn: (args: TArgs) => Promise<unknown>
) {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: fn,
    onSuccess: (data) => {
      // Every mutation returns a fresh snapshot; seed the cache with it.
      if (data && typeof data === 'object' && 'sources' in (data as object)) {
        qc.setQueryData(ROOT, data);
      } else {
        qc.invalidateQueries({ queryKey: ROOT });
      }
      // Entry changes (add/import/remove/refresh) shift the known-backbone set.
      qc.invalidateQueries({ queryKey: [...ROOT, 'backbones'] });
    },
  });
}

// Distinct backbone root domains the instance has seen — options for the picker.
export function useKnownBackbones() {
  return useQuery({
    queryKey: [...ROOT, 'backbones'],
    queryFn: () => api<string[]>('/screener/backbones'),
    staleTime: 60_000,
  });
}

export function useSetScreenerSettings() {
  return useScreenerMutation((args: Partial<ScreenerSettings>) =>
    api<ScreenerSnapshot>('PUT /screener/settings', {
      body: args as unknown as Record<string, unknown>,
    })
  );
}

export interface AddRemoteArgs {
  url: string;
  name?: string;
  trust?: Trust;
  refreshSeconds?: number;
}
export function useAddRemoteSource() {
  return useScreenerMutation((args: AddRemoteArgs) =>
    api<ScreenerSnapshot & { id: string; status: string }>(
      'POST /screener/sources/remote',
      { body: args as unknown as Record<string, unknown> }
    )
  );
}

export interface BulkAddArgs {
  input: string;
  trust?: Trust;
  refreshSeconds?: number;
  expand?: boolean;
}
export interface BulkAddResult extends ScreenerSnapshot {
  added: number;
  skipped: number;
  invalid: number;
  expandedFrom?: string;
  truncated: boolean;
}
export function useAddRemoteSourcesBulk() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (args: BulkAddArgs) =>
      api<BulkAddResult>('POST /screener/sources/remote/bulk', {
        body: args as unknown as Record<string, unknown>,
      }),
    onSuccess: (data) => {
      qc.setQueryData(ROOT, data);
      qc.invalidateQueries({ queryKey: [...ROOT, 'backbones'] });
      // New sources fetch in the background; re-pull a couple of times so their
      // counts and statuses land without the user hitting refresh.
      for (const delay of [2000, 5000, 9000]) {
        setTimeout(() => qc.invalidateQueries({ queryKey: ROOT }), delay);
      }
    },
  });
}

export interface UpdateSourceArgs {
  id: string;
  enabled?: boolean;
  trust?: Trust;
  refreshSeconds?: number;
  name?: string;
}
export function useUpdateSource() {
  return useScreenerMutation(({ id, ...patch }: UpdateSourceArgs) =>
    api<ScreenerSnapshot>(`PATCH /screener/sources/${encodeURIComponent(id)}`, {
      body: patch as Record<string, unknown>,
    })
  );
}

export function useRemoveSource() {
  return useScreenerMutation(({ id, clear }: { id: string; clear?: boolean }) =>
    api<ScreenerSnapshot>(
      `DELETE /screener/sources/${encodeURIComponent(id)}${clear ? '?action=clear' : ''}`
    )
  );
}

export function useRefreshSource() {
  return useScreenerMutation((id: string) =>
    api<ScreenerSnapshot & { status: string }>(
      `POST /screener/sources/${encodeURIComponent(id)}/refresh`
    )
  );
}

export interface ImportArgs {
  content: string;
  target: 'merge' | 'separate';
  name?: string;
  trust?: Trust;
}
export function useImportList() {
  return useScreenerMutation((args: ImportArgs) =>
    api<ScreenerSnapshot & { added: number; invalid: number }>(
      'POST /screener/import',
      { body: args as unknown as Record<string, unknown> }
    )
  );
}

export interface MarkArgs {
  key: string;
  verdict: 'dead' | 'fake' | 'mislabeled';
}
export function useMarkVerdict() {
  return useScreenerMutation((args: MarkArgs) =>
    api<ScreenerSnapshot>('POST /screener/mark', {
      body: args as unknown as Record<string, unknown>,
    })
  );
}

export interface GitHubSyncView {
  enabled: boolean;
  owner: string;
  repo: string;
  branch: string;
  path: string;
  format: 'native' | 'davex';
  scope: 'local' | 'all';
  intervalSeconds: number;
  hasToken: boolean;
  rawUrl: string | null;
  lastSyncAt: number | null;
  lastStatus: string | null;
}

export interface GitHubSyncInput {
  enabled?: boolean;
  owner?: string;
  repo?: string;
  branch?: string;
  path?: string;
  format?: 'native' | 'davex';
  scope?: 'local' | 'all';
  intervalSeconds?: number;
  // undefined keeps the stored token; '' clears it; a string sets it.
  token?: string | null;
}

const GITHUB_SYNC = ['screener', 'github-sync'] as const;

export function useGitHubSync() {
  return useQuery({
    queryKey: GITHUB_SYNC,
    queryFn: () => api<GitHubSyncView>('/screener/sync/github'),
    staleTime: 10_000,
  });
}

export function useSetGitHubSync() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (args: GitHubSyncInput) =>
      api<GitHubSyncView>('PUT /screener/sync/github', {
        body: args as Record<string, unknown>,
      }),
    onSuccess: (data) => qc.setQueryData(GITHUB_SYNC, data),
  });
}

export function useRunGitHubSync() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: () =>
      api<GitHubSyncView & { status: string }>(
        'POST /screener/sync/github/run'
      ),
    onSuccess: (data) => qc.setQueryData(GITHUB_SYNC, data),
  });
}

/** Direct download URL for an export (auth cookie rides along). */
export function exportUrl(
  scope: 'local' | 'all',
  format: 'native' | 'davex',
  dedup: boolean
): string {
  const params = new URLSearchParams({
    scope,
    format,
    dedup: dedup ? '1' : '0',
  });
  return `/api/v1/screener/export?${params.toString()}`;
}
