...
Along with `query()`, `header()`, `includes()` and `matches()`, described below.
...
A query parameter from the request, or `''`. **Variant conditions only.**
...
A request header, or `''`. Names are case-insensitive. Credentials and internal headers are not exposed. **Variant conditions only.**
...
Whether one string contains another, ignoring case. **Variant conditions only.**
...
Whether a regular expression matches a value. The pattern must be a quoted literal, and this instance's regex rules apply. Wrap it as `/pattern/flags` to pass flags, the same as every other regex field. **Variant conditions only.**
