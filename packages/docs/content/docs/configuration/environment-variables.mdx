...
| `USENET_DEFAULT_PIPELINE_DEPTH` | Default Pipeline Depth | number | `1` | Default number of in-flight BODY/STAT commands per connection (NNTP pipelining). `1` (default) is sequential — today’s behaviour. Higher hides per-article round-trip latency so fewer connections saturate a fast/high-latency link; a per-provider value overrides this. Falls back to 1 automatically if a provider mishandles pipelining. |
...
| `USENET_FAIL_ARCHIVED_RESULTS` | Fail Archived Results | boolean | `false` | Treat any archived (RAR/7z) result as non-streamable and skip it entirely, rather than attempting stored-archive streaming. |
| `USENET_FAIL_NESTED_ARCHIVES` | Fail Nested Archives | boolean | `false` | When a streamable file is itself nested inside another archive, fail instantly instead of opening the nested archive (which adds first-byte latency). Enabled by default: best-effort nested playback (one level, RAR/7z either direction). Set true to prefer fast failover instead. |
