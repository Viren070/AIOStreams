...
| Environment Variable | UI Setting | Type | Default | Description |
| --- | --- | --- | --- | --- |
| `DEEPBRIDGE_URL` | Deepbridge › URL | list | — | Upstream URL(s) for the DeepBridge addon. |
| `DEFAULT_DEEPBRIDGE_TIMEOUT` | Deepbridge › Default Timeout | number | — | Default timeout for the DeepBridge addon (milliseconds). Falls back to the global default when unset. |
| `DEFAULT_DEEPBRIDGE_USER_AGENT` | Deepbridge › Default User Agent | string | — | Default User-Agent for the DeepBridge addon. Supports `{version}`/`{random}` placeholders. |
