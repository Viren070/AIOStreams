...
| `USENET_VERIFY_MODE` | Verify Mode | enum | `stat` | How the chosen video is checked for retrievability at import, before a stream URL is minted. `stat` (default) is a fast existence check, but some cache/debrid NNTP gateways answer “present” for articles they cannot actually deliver, so a dead release can slip through. `body` actually fetches the sample segments — authoritative (catches those gateways and incomplete posts) and the fetched segments are cached as start-of-playback prefetch, at a small first-byte latency cost. `none` skips this check. |
