...
| `USENET_WEBDAV_ENABLED` | Webdav Enabled | boolean | `true` | Expose the built-in usenet library as a read-only WebDAV filesystem at `/dav` so media players (Infuse, Kodi, VLC) and *arr completed-download mounts can browse and stream it on demand, with zero local storage. Two trees are served: `/dav/library/<category>/<release>` (matches the SABnzbd-compatible API’s reported paths, for Sonarr/Radarr import) and `/dav/media/Movies\|Series` (clean, parsed names for browsing). Log in with an `AIOSTREAMS_AUTH` `username:password`. Inert until NNTP providers and `AIOSTREAMS_AUTH` are configured. |
