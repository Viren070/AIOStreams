...
| `RELEASE_BLOCKLIST_GITHUB_TOKEN`           | Github Token           | string  | _(unset)_  | When set, this instance's own verdicts can be published to a secret GitHub gist and shared as a `gist.githubusercontent.com` URL, so subscribers never see your instance's address and it does not need to be reachable from the internet. Needs a token with the `gist` scope. _(secret)_                                                                                                                                                                                                                                                                                                                                                                                    |
| `RELEASE_BLOCKLIST_GITHUB_GIST_URL`        | Github Gist URL        | string  | —          | The raw URL of the gist the blocklist is published to. Filled in automatically on the first publish; set it yourself to reuse an existing gist.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
