...
description: Overview of the configuration page across all tabs.
---
...
<Callout type="info">
  ...
  find it [here](/docs/migrations/v1-to-v2).
</Callout>
...
<Callout type="info">
  ...
  You will either need to add your UUID to the `TRUSTED_UUIDS` environment
  variable, or set the `REGEX_FILTER_ACCESS` environment variable to `all` (only
  recommended if you have set an `ADDON_PASSWORD`, ensuring no one unauthorised
