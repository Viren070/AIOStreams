...
description: A step-by-step guide to setting up AIOStreams.
---
...
2.  Now, order them as follows in the `Order` section using the grab handles: - `Cached` → `Library` → `Resolution` → `Quality` → `Regex Patterns` → `Visual Tag` → `Language` → `Size` → `Seeder`
...
<Callout type="info">
  Click `Export` in the `Backups` section to download a backup of your
  configuration. You can use this to restore your configuration later if you
  need to.
</Callout>
