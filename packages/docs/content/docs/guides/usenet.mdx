...
[**InfiniDysk**](https://github.com/infinidysk/infinidysk) (the NzbDAV superfork) can also own search itself. Add the **InfiniDysk** marketplace preset and paste a Search Profile `manifest.json` URL. In that mode you do not add the InfiniDysk/NzbDAV service or duplicate those indexers as Newznab addons. AIOStreams still filters and sorts the returned streams and reports the final order back to InfiniDysk.
