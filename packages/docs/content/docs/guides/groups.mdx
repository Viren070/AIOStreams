...
Group conditions are written using the [Stream Expression Language (SEL)](/docs/reference/stream-expressions). The condition must evaluate to `true` or `false`.
...
For the complete list of available functions, see the [SEL reference](/docs/reference/stream-expressions).
