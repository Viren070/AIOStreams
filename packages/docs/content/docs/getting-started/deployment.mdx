...
<Callout type="info">
  ...
  All others are optional. See the full list in [Environment
  Variables](/docs/configuration/environment-variables).
</Callout>
...
<Steps>
  ...
  <Step>
    Edit `.env` to set at minimum a `SECRET_KEY` (64-character hex string). See [Environment Variables](/docs/configuration/environment-variables).
  </Step>
  <Step>
    Start the container:
