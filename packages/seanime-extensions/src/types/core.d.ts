...
declare class Buffer extends ArrayBuffer {
  ...
  constructor(
    arg?: string | ArrayBuffer | ArrayLike<number>,
    encoding?: string
  );
...
declare class DocSelection {
  ...
  data<T extends string | undefined>(
    name?: T
  ): T extends string ? string | undefined : { [key: string]: string };
  ...
  filter(
    selector: string | ((index: number, element: DocSelection) => boolean)
  ): DocSelection;
...
declare namespace $store {
   ...
   */
  function setIfLessThanLimit<T = any>(
    key: string,
    value: T,
