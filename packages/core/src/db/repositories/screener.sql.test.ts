...
import { screener } from '../migrations/0013_screener.js';
import { screenerRefreshSeconds } from '../migrations/0014_screener_refresh_seconds.js';
...
before(async () => {
  ...
  // 0014 is an imperative (idempotent) migration; drive it through a minimal
  // sqlite context so the test exercises the real guarded rename path.
  const tx = {
    dialect: 'sqlite' as const,
    exec: async (sql: string) => db.exec(sql),
    maybeOne: async (sql: string) => (db.prepare(sql).get() ?? null) as any,
  };
  await screenerRefreshSeconds.run!(tx);
});
after(() => {
  db.close();
