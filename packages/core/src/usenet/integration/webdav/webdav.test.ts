...
describe('webdav paths', () => {
  ...
  test('assignReleaseFolders disambiguates identical release names', () => {
    ...
    assert.equal(folders[0], 'Same.Release');
    assert.notEqual(folders[1], folders[0]);
    assert.match(folders[1], /^Same\.Release-/);
  });

  test('webdavLibraryStoragePath matches the library tree path', () => {
    assert.equal(
      webdavLibraryStoragePath(MOVIE),
      '/dav/library/movies/Big.Movie.2024.2160p.mkv'
    );
  });
...
describe('webdav tree resolution round-trip', () => {
  ...
  test('media Series tree buckets an episode by show + season', async (t) => {
    ...
    assert.equal(eps!.children[0].kind, 'file');
  });

  test('media Movies tree exposes a movie folder', async (t) => {
    mockList(t, [entry(), MOVIE]);
