...
/** Most entries the WebDAV tree will surface in one listing (bounded query). */
...
/**
 * Load every available library entry that has at least one browsable file. This
 * is the single source both trees are projected from, so the library and media
 * views never disagree about what exists. Bounded by {@link MAX_LIBRARY_ENTRIES};
 * a truncated load is logged rather than silently capped.
 */
async function loadAvailableEntries(): Promise<UsenetLibraryEntry[]> {
  ...
  if (total > entries.length) {
    logger.warn(
      { total, shown: entries.length, cap: MAX_LIBRARY_ENTRIES },
      'webdav library listing truncated; some entries are not browseable over WebDAV'
    );
  }
  return entries.filter((e) => e.files.some(isBrowsableFile));
}
...
function mediaTriples(
  entries: UsenetLibraryEntry[]
): Array<{ entry: UsenetLibraryEntry; file: UsenetLibraryFile; cls: MediaClass }> {
  const out: Array<{
    entry: UsenetLibraryEntry;
    file: UsenetLibraryFile;
    cls: MediaClass;
...
/** De-duplicate sibling file names within a media folder (best-effort). */
...
export async function resolveWebdavNode(
  ...
  if (root === WEBDAV_LIBRARY_ROOT) return resolveLibrary(rest);
  if (root === WEBDAV_MEDIA_ROOT) return resolveMedia(rest);
  return undefined;
}

async function resolveLibrary(rest: string[]): Promise<WebdavResolved | undefined> {
  const entries = await loadAvailableEntries();
...
async function resolveMedia(rest: string[]): Promise<WebdavResolved | undefined> {
  ...
  const entries = await loadAvailableEntries();
  const triples = mediaTriples(entries);
...
function resolveSeries(
  ...
  const showFolder = (t: (typeof series)[number]) => sanitizeSegment(t.cls.title);
