...
describe('overlapping bare-episode aliases', () => {
  it('preserves the full release identity when a shorter suffix appears first', async (t) => {
    t.mock.method(SettingsRepository, 'getAll', async () => []);
    t.mock.method(SettingsRepository, 'getVersion', async () => 0);
    await settingsStore.initialise();
    const context = {
      ...
      parsedId: { season: '1', episode: '4' },
      getMetadata: async () => ({
        title: 'Distinctive Shared Name',
        titles: [{ title: 'Distinctive Shared Name' }],
    ...
    const streams = [
      'Distinctive Shared Name 04.mkv',
      'Shared Name 04.mkv',
    ].map((filename, i) => ({
      id: String(i),
      type: 'p2p',
    ...
    const result = await new StreamFilterer({
      titleMatching: { enabled: true, mode: 'exact', similarityThreshold: 1 },
      seasonEpisodeMatching: { enabled: true, strict: true },
    } as UserData).filter(streams, context);
    assert.deepEqual(
