...
import { ProwlarrApiSearchItemSchema } from './prowlarr/api.js';
...
describe('yearless movie fallback', () => {
  it('retains indexer identity when trimming Prowlarr API results', () => {
    const result = ProwlarrApiSearchItemSchema.parse({
      ageHours: 1,
      size: 123,
      indexer: 'Example',
      indexerId: 42,
    ...
    assert.equal(result.indexerId, 42);
    assert.equal('unusedField' in result, false);
  });
    ...
    Object.assign(addon, {
      ...
      api: {
        search: async (params: { query: string; indexerIds: number[] }) => {
          calls.push(params);
          return params.query.endsWith('2026')
            ? [
                ...['a', 'b', 'c'].map((guid) => ({
                  ...item(guid),
    ...
    it(`Prowlarr counts only usable ${protocol} results per indexer`, async () => {
      ...
      Object.assign(addon, {
        ...
        api: {
          search: async (params: { query: string; indexerIds: number[] }) => {
            calls.push(params);
            return params.query.endsWith('2026') ? initial : [];
          },
        },
      });
