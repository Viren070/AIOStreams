...
const STREMTHRU_TORRENT_SERVICE = 'stremthru_torrent';
...
> = {
  ...
  [STREMTHRU_TORRENT_SERVICE]: {
    id: STREMTHRU_TORRENT_SERVICE,
    name: 'StremThru (Torrents)',
    shortName: 'ST',
    knownNames: ['ST', 'StremThru Torrent', 'StremThruTorrent'],
    signUpText:
      'Stream torrents via a custom [StremThru](https://github.com/MunifTanjim/stremthru) instance — point it at any StremThru-compatible server (e.g. a self-hosted one) instead of the shared instance.',
    credentials: [
      {
        id: 'url',
        name: 'StremThru URL',
        description:
          'The base URL of the StremThru instance used for requests e.g. http://stremthru:8080 or https://stremthru.mydomain.com',
        type: 'string',
        required: true,
      },
      {
        ...
        name: 'Auth Token',
        description:
          'The StremThru authentication token (`STREMTHRU_AUTH`), or the debrid API token when the instance fronts a debrid store.',
        type: 'password',
        required: true,
      },
      {
        ...
        name: 'Store Name',
        description:
          'Optional. The StremThru store to use (e.g. `realdebrid`, `alldebrid`). Leave blank when the instance itself is the store.',
        type: 'string',
        required: false,
      },
    ],
