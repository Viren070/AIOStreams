...
- `packages/desktop` — the desktop app (Windows only so far): a Cargo workspace, outside the pnpm build, that shows `jellyfin-web` in a transparent WebView2 over libmpv and exposes a checked JSON bridge to mpv as `window.aiostreamsDesktop`. See its README.
