...
- `packages/jellyfin-web` — the web app served at every Jellyfin mount's `/web` (rsbuild, own `dist`). The server sends its `index.html` from each mount and serves its hashed assets at `/jellyfin-web/static`. Built for AIOStreams-like Jellyfin servers (direct play only, optional `aiostreams` extension fields), not as a general Jellyfin client. `build:standalone` (`--env-mode standalone`, the `__STANDALONE__` constant) builds a copy that no server hosts and that picks its own server, into `dist-standalone`; the desktop app serves that one.
- `packages/desktop` — the desktop app (Windows, Linux and macOS): a Cargo workspace, outside the pnpm build, that serves `jellyfin-web`'s standalone build in a transparent web view (WebView2 via tao/wry on Windows, WebKitGTK 6 via GTK 4 on Linux, WKWebView via tao/wry on macOS) over libmpv and exposes a checked JSON bridge to mpv as `window.aiostreamsDesktop`. See its README.
...
- UI: components come from `@aiostreams/ui` (Tailwind 3 + Radix primitives + `class-variance-authority` for variants); `framer-motion`/`motion` for animation.
